      ...
      <div className="relative min-h-[200px] w-full max-w-[400px] md:max-w-[500px] lg:max-w-2xl bg-[var(--color-4)]/20 rounded-lg overflow-hidden p-4 bord hover:scale-105 transition-scale ease-in-out duration-200 card">
        ...
        <a href={`/products/${id}`} className="w-full md:w-96 h-auto max-h-[200px] object-cover pointer">
          <img
            src={imgUrl}
            alt={title}
            className="w-full md:w-96 h-full max-h-[200px] object-cover rounded-lg border-2 border-[var(--color-1)]"
          ...
          <div className="flex flex-col justify-between w-full">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-end md:mr-16">
              <button
                onClick={toggleModal}
                className="px-4 py-2 rounded-md bg-[var(--color-1)] hover:bg-[var(--color-3)]/90 transition-colors ease-in-out duration-200 text-white w-full sm:w-auto pointer"
              >
                add to cart
              </button>
              <a href={`/products/${id}`} className="w-full sm:w-auto pointer-events-auto">
                <button className="px-4 py-2 rounded-md bg-[var(--color-2)]/70 hover:bg-[var(--color-2)]/90 transition-colors ease-in-out duration-200 text-white w-full pointer" >
                  see more details
                </button>
              </a>
            </div>
